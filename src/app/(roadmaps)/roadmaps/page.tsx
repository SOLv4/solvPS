      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 p-6">
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-400">DB에 있는 모든 로드맵을 조회/생성/삭제합니다.</p>
              </div>
              ...
              <Dialog open={createOpen} onOpenChange={setCreateOpen}>
                ...
                <DialogContent className="rounded-2xl">
                  <DialogHeader>
                    ...
                    <DialogDescription>생성 후 문제 검색 페이지에서 바로 담을 수 있습니다.</DialogDescription>
                  </DialogHeader>
                  <div className="space-y-4">
                    ...
                    {groups.length > 0 ? (
                      <p className="text-xs text-gray-500">
                        연결 그룹: {groups.find((group) => group.id === selectedGroupId)?.name ?? "-"}
                      </p>
                    ) : (
                      <p className="text-xs text-gray-500">연결 가능한 그룹이 없습니다. 그룹 가입 후 생성하세요.</p>
                    )}
                  </div>
                  <DialogFooter>
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {sortedRoadmaps.map((roadmap, index) => {
            ...
            const detailHref = roadmap.teamId ? `/roadmaps/${roadmap.id}?groupId=${roadmap.teamId}` : "";
              ...
              <article key={roadmap.id} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-colors hover:border-gray-200">
                <div className="mb-3 flex items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    <p className="text-[11px] text-slate-500">{roadmap.teamName ?? "그룹 미연결"}</p>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {detailHref ? (
                      ...
                      <Button size="sm" variant="outline" className="rounded-lg border-gray-200 text-gray-400" disabled>
                        상세 보기
                      </Button>
                    )}
                    <Button
